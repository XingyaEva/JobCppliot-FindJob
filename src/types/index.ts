...
// ==================== Phase A7: 用户 Dashboard ====================

/** 用户活动记录 */
export interface UserActivity {
  id: string;
  type: 'parse' | 'match' | 'resume' | 'interview' | 'offer' | 'apply' | 'optimize' | 'question' | 'other';
  title: string;                      // 活动描述
  relatedEntityId?: string;           // 关联实体 ID（job/resume/application）
...
export interface WeeklyGoal {
  ...
  weekStart: string;                  // 所属周的起始日期（ISO，用于按周筛选）
  autoGenerated: boolean;             // 是否系统自动生成
  created_at: string;
  updated_at: string;
}
...
export interface TodoItem {
  id: string;
  text: string;                       // 待办描述
  source: TodoSource;                 // 来源：auto=自动推导 / manual=手动创建
  priority: TodoPriority;             // 优先级
  completed: boolean;                 // 是否完成
  dismissed: boolean;                 // 是否已关闭（仅 auto 类型）
  dueTime?: string;                   // 截止时间（ISO）
  relatedType?: 'job' | 'application' | 'resume' | 'interview' | 'goal'; // 关联实体类型
  relatedId?: string;                 // 关联实体 ID
  ruleName?: string;                  // 自动推导规则名（用于去重）
  created_at: string;
  updated_at: string;
}
...
/** Dashboard 能力雷达 */
export interface DashboardSkillDimension {
  dimension: string;
  score: number;
  fullMark: number;
}
